...
namespace CreatorKitCode {
    ...
    /// </summary>
    [CreateAssetMenu(fileName = "QuestItem", menuName = "Beginner Code/Quest Item", order = -999)]
...
public class QuestItemEditor : Editor
{
    ...
    void OnEnable()
    {
        m_Target = target as QuestItem;
        m_UsageEffectListProperty = serializedObject.FindProperty(nameof(QuestItem.UsageEffects));
        
        m_ItemEditor = new ItemEditor();
        m_ItemEditor.Init(serializedObject);

        var lookup = typeof(QuestItem.UsageEffect);
